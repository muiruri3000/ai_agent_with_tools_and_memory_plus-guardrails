...
class Atlas:
    ...
    def handle_customer_update(self, message: str) -> str:
        ...
        if update["customer_id"] is not None:
            ...
            customer = get_customer_by_id(update["customer_id"])

            if not customer:
                return f"I could not find customer " f"ID {update['customer_id']}."
        ...
        # ---------------------------------------------
            ...
            results = find_customer(name=update["name"])
        ...
        # ---------------------------------------------
        # REPORT SUCCESS + REFRESH CONTEXT
        # ---------------------------------------------
    ...
    def handle_customer_delete(self, message: str) -> str:
        ...
        if (
            current_customer
            and current_customer["id"] == deleted["id"]
        ):
            self.clear_customer_context()

        return (
            f"{deleted['name']} has been successfully "
